...
async function testWithCourseId() {
    console.log("--- Testing CRM Insert with Valid courseId ---");

    const payload = {
        name: "Test with CourseId",
        phone: "1234567890",
        email: "tc@test.com",
        courseId: 1, // Full Stack Development
        status: "NEW",
        source: "Sync Debug",
    ...
    if (error) {
        console.error("❌ Insert with courseId Failed:", error.message);
        console.error("Details:", JSON.stringify(error, null, 2));
    } else {
        console.log("✅ Insert with courseId Success!");
